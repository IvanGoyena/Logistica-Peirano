...
def _preparar_fechas_oc(
    tabla_pendientes_oc: pd.DataFrame | None,
) -> pd.DataFrame:
    """Normaliza una referencia única de próxima OC por artículo."""
    columnas = [
        "ArticuloCodigo",
        "OrdenCompraProxima",
    ...
    if tabla_pendientes_oc is None or tabla_pendientes_oc.empty:
        return pd.DataFrame(columns=columnas)

    origen = tabla_pendientes_oc.copy()
    col_codigo = _buscar_columna(
        origen,
        ["ArticuloCodigo", "CodigoArticulo", "cod_art", "Articulo"],
        obligatoria=False,
    )
    if col_codigo is None:
        return pd.DataFrame(columns=columnas)

    col_oc = _buscar_columna(
        origen,
        ["OrdenCompraProxima", "OrdenCompra", "OC", "nro_com"],
        obligatoria=False,
    )
    ...
    col_tipo = _buscar_columna(
        origen,
        ["TipoFechaIngresoOC", "TipoFechaIngreso"],
        obligatoria=False,
    )

    salida = pd.DataFrame({
        "ArticuloCodigo": origen[col_codigo].map(_normalizar_codigo),
        "OrdenCompraProxima": (
            origen[col_oc].map(_normalizar_texto) if col_oc else ""
        ),
        "FechaPrevistaIngresoOC": (
            pd.to_datetime(origen[col_fecha], errors="coerce", dayfirst=True)
            if col_fecha
            else pd.NaT
        ),
        "TipoFechaIngresoOC": (
            origen[col_tipo].fillna("").astype(str).str.strip()
            if col_tipo
            else ""
        ),
    })
...
def analizar_cobertura_pedidos_erp(
    ...
    tabla_pendientes_oc: pd.DataFrame | None = None,
    ...
    fechas_oc = _preparar_fechas_oc(tabla_pendientes_oc)
    lineas = lineas.merge(
        fechas_oc,
        on="ArticuloCodigo",
        how="left",
    ...
    lineas["OrdenCompraProxima"] = (
        lineas.get("OrdenCompraProxima", "")
        .fillna("")
        .astype(str)
        .str.strip()
    )
    lineas["FechaPrevistaIngresoOC"] = pd.to_datetime(
        lineas.get("FechaPrevistaIngresoOC", pd.NaT),
        errors="coerce",
    )
    ...
    # La fecha de OC solo es relevante para líneas que continúan sin cobertura
    # luego de considerar el tránsito ERP.
    sin_faltante_real = lineas["FaltanteLuegoTransito"].le(0)
    lineas.loc[sin_faltante_real, "OrdenCompraProxima"] = ""
    lineas.loc[sin_faltante_real, "FechaPrevistaIngresoOC"] = pd.NaT

    hoy = pd.Timestamp.today().normalize()
    dias_ingreso_oc = (
        lineas["FechaPrevistaIngresoOC"].dt.normalize() - hoy
    ).dt.days
    lineas["EstadoIngresoOC"] = ""
    con_fecha_oc = lineas["FechaPrevistaIngresoOC"].notna()
    lineas.loc[con_fecha_oc & dias_ingreso_oc.lt(0), "EstadoIngresoOC"] = "Atrasado"
    lineas.loc[con_fecha_oc & dias_ingreso_oc.between(0, 7), "EstadoIngresoOC"] = "Dentro de 7 días"
    lineas.loc[con_fecha_oc & dias_ingreso_oc.between(8, 15), "EstadoIngresoOC"] = "Entre 8 y 15 días"
    lineas.loc[con_fecha_oc & dias_ingreso_oc.gt(15), "EstadoIngresoOC"] = "Más de 15 días"
    ...
    resumen = (
        ...
        .agg(
            ...
            ProximoIngresoOC=("FechaPrevistaIngresoOC", "min"),
            CodigosConFaltante=(
                "ArticuloCodigo",
                lambda serie: int(
                    lineas.loc[
    ...
    dias_resumen_oc = (
        resumen["ProximoIngresoOC"].dt.normalize()
        - pd.Timestamp.today().normalize()
    ).dt.days
    resumen["EstadoIngresoOC"] = ""
    con_fecha_resumen = resumen["ProximoIngresoOC"].notna()
    resumen.loc[con_fecha_resumen & dias_resumen_oc.lt(0), "EstadoIngresoOC"] = "Atrasado"
    resumen.loc[con_fecha_resumen & dias_resumen_oc.between(0, 7), "EstadoIngresoOC"] = "Dentro de 7 días"
    resumen.loc[con_fecha_resumen & dias_resumen_oc.between(8, 15), "EstadoIngresoOC"] = "Entre 8 y 15 días"
